...
function MembersScrollCards({ data }: MembersProps) {
    ...
    return (
        <>
            <div className='w-full h-full flex-wrap gap-14 gap-x-28 sm:gap-x-28 lg:gap-x-60 flex justify-center sm:justify-between md:justify-evenly'>
                {
                    data && showCards && data?.map((item: any, index: number) => {
                        return (
                ...
                {
                    data && !showCards && <div className={`md:hidden read-more z-10 absolute flex items-end pb-5 justify-center bg-black w-full h-52 left-0 bottom-0`}>
